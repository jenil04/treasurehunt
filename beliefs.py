...
class Belief(object):
    ...
    def update(self, color, sensor_position, model):
        ...
        for pos, prob in self.current_distribution.items():
            self.current_distribution[pos] = prob * model.pcolorgivendist(color, utils.manhattan_distance(pos, sensor_position))

        z = sum(self.current_distribution.values())

        for pos, prob in self.current_distribution.items():
            self.current_distribution[pos] = float(prob/z)
    ...
    def recommend_sensing(self):
        ...
        reccomendation = max(self.open, key=lambda p: self.current_distribution[p])

        if self.current_distribution[reccomendation] == 0:
            temp = max(self.current_distribution, key=lambda pos: self.current_distribution[pos])
            reccomendation = utils.closest_point(temp, self.open)
